...
def save_calculation(filename, data, equivalences, coeffs, metadata):
    ...
    with lzma.open(filename, "wt", encoding="utf-8", preset=0) as f:
        ...
        f.write(
            json.dumps(
                [data, equivalences, coeffs, metadata], cls=JSONEncoder))
...
def save_results(filename, data, fermi, Tr, mu0, mur, N, sdos, cv, cond,
                 seebeck, kappa, hall, metadata):
    ...
    with lzma.open(filename, "wt", encoding="utf-8", preset=0) as f:
        f.write(
            json.dumps(
                [
                    data, fermi, Tr, mu0, mur, N, sdos, cv, cond, seebeck,
                    kappa, hall, metadata
                ],
                cls=JSONEncoder))
